...
const wowheadcall = (i) =>
  axios.get("https://www.wowhead.com/item=" + i + "&xml", {}).then(function (response) {
    ...
    insertValues(json.wowhead.item._attributes.id, json.wowhead.item.name._cdata.replaceAll('"', "'"), json.wowhead.item.icon._text, new Date().toISOString().slice(0, 10));
    console.log("id: " + json.wowhead.item._attributes.id + " name: " + json.wowhead.item.name._cdata.replaceAll('"', "'"));
  });

const value = await sqlQuery("SELECT * from items ORDER BY id desc LIMIT 1");
console.log(value);
