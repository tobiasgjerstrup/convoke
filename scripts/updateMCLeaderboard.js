...
async function updateMCLeaderboard() {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (undefined !== JSON.parse(data).stats) {
      ...
      const res = await libs.updateTable("production", "mc_players", "'" + file.slice(0, -5) + "'", `blocks_mined = ${blocksMined}, mobs_killed = ${mobsKilled}, items_broken = ${itemsBroken}, items_dropped = ${itemsDropped}, items_picked_up = ${itemsPickedUp}, items_used = ${itemsUsed}, items_crafted = ${itemsCrafted}`);
      if (res.affectedRows < 1) {
        await libs.sqlQuery(`INSERT INTO mc_players (id, blocks_mined, mobs_killed, items_broken, items_dropped, items_picked_up, items_used, items_crafted) VALUES ('${file.slice(0, -5)}', ${blocksMined}, ${mobsKilled}, ${itemsBroken}, ${itemsDropped}, ${itemsPickedUp}, ${itemsUsed}, ${itemsCrafted})`);
      }
    }
  }
}
